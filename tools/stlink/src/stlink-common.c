...
#define STM32L0_FLASH_ACR (STM32L0_FLASH_REGS_ADDR + 0x00)
#define STM32L0_FLASH_PECR (STM32L0_FLASH_REGS_ADDR + 0x04)
#define STM32L0_FLASH_PDKEYR (STM32L0_FLASH_REGS_ADDR + 0x08)
#define STM32L0_FLASH_PEKEYR (STM32L0_FLASH_REGS_ADDR + 0x0c)
#define STM32L0_FLASH_PRGKEYR (STM32L0_FLASH_REGS_ADDR + 0x10)
#define STM32L0_FLASH_OPTKEYR (STM32L0_FLASH_REGS_ADDR + 0x14)
#define STM32L0_FLASH_SR (STM32L0_FLASH_REGS_ADDR + 0x18)
#define STM32L0_FLASH_OBR (STM32L0_FLASH_REGS_ADDR + 0x1c)
#define STM32L0_FLASH_WRPR (STM32L0_FLASH_REGS_ADDR + 0x20)
#define FLASH_L0_FPRG 10
#define FLASH_L0_PROG 3
...
int stlink_fread(stlink_t* sl, const char* path, stm32_addr_t addr, size_t size) {
    ...
    unsigned char erased_pattern = (sl->chip_id == STM32_CHIPID_L1_MEDIUM  || sl->chip_id == STM32_CHIPID_L1_MEDIUM_PLUS
                                     || sl->chip_id == STM32_CHIPID_L1_HIGH || sl->chip_id == STM32_CHIPID_L152_RE || sl->chip_id == STM32_CHIPID_L053_R8) ? 0:0xff;
...
int stlink_erase_flash_page(stlink_t *sl, stm32_addr_t flashaddr)
{
  ...
  else if (sl->chip_id == STM32_CHIPID_L053_R8 ) {

    uint32_t val;

    /* check if the locks are set */
    val = stlink_read_debug32(sl, STM32L0_FLASH_PECR);
    if((val & (1<<0))||(val & (1<<1))) {
        /* disable pecr protection */
        stlink_write_debug32(sl, STM32L0_FLASH_PEKEYR, 0x89abcdef);
        stlink_write_debug32(sl, STM32L0_FLASH_PEKEYR, 0x02030405);
        ...
        /* unlock program memory */
        stlink_write_debug32(sl, STM32L0_FLASH_PRGKEYR, 0x8c9daebf);
        stlink_write_debug32(sl, STM32L0_FLASH_PRGKEYR, 0x13141516);
    ...
    /* set pecr.{erase,prog} */
    val |= (1 << 9) | (1 << 3);
    stlink_write_debug32(sl, STM32L0_FLASH_PECR, val);
...
int stlink_erase_flash_mass(stlink_t *sl) {
	...
	|| sl->chip_id == STM32_CHIPID_L1_HIGH || sl->chip_id == STM32_CHIPID_L152_RE || sl->chip_id == STM32_CHIPID_L053_R8) {
        /* erase each page */
...
int stm32l0_write_half_pages(stlink_t *sl, stm32_addr_t addr, uint8_t* base, unsigned num_half_pages)
{
    unsigned int count;
    uint32_t val;
    flash_loader_t fl;

    ILOG("Starting Half page flash write for STM32L0 core id\n");
    /* flash loader initialization */
    if (init_flash_loader(sl, &fl) == -1) {
        WLOG("init_flash_loader() == -1\n");
        return -1;
    ...
    val |= (1 << FLASH_L0_PROG);
    stlink_write_debug32(sl, STM32L0_FLASH_PECR, val);
    while ((stlink_read_debug32(sl, STM32L0_FLASH_SR) & (1 << 0)) != 0) {}

#define L0_WRITE_BLOCK_SIZE 0x40
    for (count = 0; count  < num_half_pages; count ++) {
        if (run_flash_loader(sl, &fl, addr + count * L0_WRITE_BLOCK_SIZE, base + count * L0_WRITE_BLOCK_SIZE, L0_WRITE_BLOCK_SIZE) == -1) {
            WLOG("l1_run_flash_loader(%#zx) failed! == -1\n", addr + count * L0_WRITE_BLOCK_SIZE);
            val = stlink_read_debug32(sl, STM32L0_FLASH_PECR);
            val &= ~((1 << FLASH_L0_FPRG) |(1 << FLASH_L0_PROG));
            stlink_write_debug32(sl, STM32L0_FLASH_PECR, val);
            return -1;
        }
        /* wait for sr.busy to be cleared */
...
#define PROGRESS_CHUNK_SIZE 0x1000
	...
	off = 0;
        if (len > L1_WRITE_BLOCK_SIZE) {
            if (stm32l1_write_half_pages(sl, addr, base, len/L1_WRITE_BLOCK_SIZE) == -1) {
              /* This may happen on a blank device! */
                WLOG("\n 0x%x, 0x%x, %d write_half_pages failed == -1\n", addr, base, len/L1_WRITE_BLOCK_SIZE);
            } else {
    ...
    } else if (sl->core_id == STM32L053_CORE_ID) {
      ...
      off = 0;
      if (len > L0_WRITE_BLOCK_SIZE) {
          if (stm32l0_write_half_pages(sl, addr, base, len/L0_WRITE_BLOCK_SIZE) == -1) {
            /* This may happen on a blank device! */
              WLOG("\n 0x%x, 0x%x, %d write_half_pages failed == -1\n", addr, base, len/L0_WRITE_BLOCK_SIZE);
          } else {
            off = (len /L0_WRITE_BLOCK_SIZE)*L0_WRITE_BLOCK_SIZE;
          }
      }
      /* write remainingword in program memory */
      for ( ; off < len; off += sizeof(uint32_t)) {
...
//        ELOG("lch target %#x,fl->buf_addr  %#x, count %d, fl->loader_addr %#x\n", target, fl->buf_addr, count, fl->loader_addr);
